    ...
    <div>
      ...
      <div className="border border-gold/12 divide-y divide-gold/10">
        {posts.length === 0 && loaded && (
          <p className="p-4 text-[12px] text-muted" style={{ fontFamily: "var(--font-body-family)" }}>No posts yet.</p>
        )}
        {posts.map((p) => (
          <div key={p.id} className="flex items-center gap-4 p-4 flex-wrap">
            <div className="flex-1 min-w-[160px]">
              ...
              <p className="text-[10px] text-muted" style={{ fontFamily: "var(--font-body-family)" }}>/journal/{p.slug}</p>
            </div>
            <span className={`text-[9px] tracking-wider uppercase px-2.5 py-1 border ${p.published ? "border-gold/40 text-gold" : "border-warm-gray/20 text-muted"}`} style={{ fontFamily: "var(--font-body-family)" }}>
